...
const InterviewPractice = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.REACT_APP_BACKEND_URL}/api/interview`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleSubmit = async () => {
    try {
      const res = await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}/api/interview/submit`,
        { questionId: selectedQuestion._id, answer },
  ...
  return (
    <div className="p-8 flex flex-col md:flex-row gap-4">
      ...
      {selectedQuestion && (
        <div className="w-full md:w-1/2">
          <h2>
            {selectedQuestion.question} ({selectedQuestion.type})
          </h2>
          <textarea
            value={answer}
